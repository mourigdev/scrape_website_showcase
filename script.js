  ...
  mainData.forEach(async (item, i) => {
    if (i !== 0) {
      ...
      await getData(`${mainApiURL}?id=${item[0]}`,async (detailsData)=>{
        ...
        <div class="elementor-element elementor-element-349f58c elementor-widget elementor-widget-text-editor" data-id="349f58c" data-element_type="widget" data-widget_type="text-editor.default" style="text-align:end">
        <div class="elementor-widget-container" style="margin:0 8px;color:black">
        Job ID# ${mainData[i][0]}</div>
        </div>
        </section>
        </div>
        </div></div>`;
